...
function displayResultsOnNewPage(results) {
    ...
    let content = '<h1>NO2 Levels Query Results</h1>';
    content += '<table border="1"><tr><th>City</th><th>County</th><th>Year</th><th>NO2 Level</th></tr>';
...
const submitQuery = async () => {
    try {
        ...
        SELECT ?city ?county ?year (SAMPLE(?NO2Value) AS ?uniqueNO2Value)
        FROM <${graphUri}>
        WHERE {
            ...
            ?metric rdf:type project:NO2.
            ?metric project:hasComposition ?NO2Value.
            ?metric project:reportedIn ?reportYear_x.
            ?reportYear_x rdf:type project:ReportYear.
            ?reportYear_x project:hasReportYear ?year.
            # Add a filter for the state (replace YourState with the actual state value)
